...
class SimGenerator(object):
    ...
    def __init__(
        ...
        data_dir='./sim_data', debug=False, worker_num=4, sim_count=2500
    ):
        ...
        with futures.ProcessPoolExecutor(max_workers=self._worker_num) as executor:
            while current_count < self._sim_count:
                future_to_idx = dict((executor.submit(sim_run_func, {
                    ...
                    'debug': self._debug,
                }), current_count + i) for i in range(batch_size))
                _results = []
                for future in futures.as_completed(future_to_idx):
                    idx = future_to_idx[future]
                    if future.exception():
                        logger.error('Sim[{i}] error: {e}'.format(i=idx, e=future.exception()))
                        continue
